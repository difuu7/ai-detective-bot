...
# ========== НАСТРОЙКИ ==========
TOKEN = "8514983133:AAF4dvNmweMg8LOBVB2evu_bw3td3d_p8jM"  # ТВОЙ ТОКЕН
...
def update_user_stats(user_id, username, image_id, guess, correct_label, response_time):
    ...
    points = 20 if is_correct else -5  # Удвоенные очки для челленджа
...
def handle_answer(call):
    ...
    # Обновляем статистику (с удвоенными очками для челленджа)
    is_correct, points, streak = update_user_stats(
        user_id, username, image_id, guess, correct, response_time
    )
    
    # Если это ежедневный челлендж, показываем бонус
    if is_daily:
        points_text = f"+{points} очков (x2 бонус)"
    else:
        points_text = f"{points} очков"
    ...
    result += f"\n💰 {points_text} | ⏱ {response_time:.1f} сек\n\n"
